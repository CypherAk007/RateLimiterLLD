...
public class UserTokenBucket implements Client{
    private final ConcurrentHashMap<Integer, TokenBucket> bucket;

    public UserTokenBucket(int clientId) {
        this.bucket = new ConcurrentHashMap<>();
        bucket.put(clientId,new TokenBucket(5,1));
    }
